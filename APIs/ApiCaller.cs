...
namespace Movie_API
{
    public class WebRequest
    {
        ...
        public static async Task GetMovieDataAsync(string searchStr, Action<Dictionary<string, object>> Callback)
        {
            ...
            using (var Client = new HttpClient())
            {
                try
                {
                    Client.BaseAddress = new Uri($"https://api.themoviedb.org/3/search/movie?api_key=f280f7c3732f01b778a4f9edabad8961&language=en-US&query={searchStr}&page=1&include_adult=false");
